...
class ProfileTest < ActiveSupport::TestCase
  ...
  test 'Profile must have a user and a name ' do
    p = Profile.new
    assert_not p.valid?, 'Profile should not be valid without a name or user'
  ...
  test 'Profile cannot link to the same provider twice ' do
    ...
    new_pp = ProfileProvider.new(provider_id: pp.provider_id,
                                 ...
                                 subject_id: '')
    assert_not new_pp.valid?, 'Provider Profile should not be valid if the profile is already linked to the provider'
  ...
  test 'Profile cannot link to provider if already linked to parent' do
    ...
    assert_not new_pp.valid?, 'Provider Profile should not be valid if the profile is already linked to the providers parent'
